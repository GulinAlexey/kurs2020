...
public:
	...
	void Save_game(); //сохранение игры (запись информации о деревне в файл)
	void Init_load_game(); //инициализация при загрузке сохранённой игры
	void Delete_krest(int number); //удаление крестьянина с выбранным номером (нумерация от единицы!)
	void Add_rand_krest(); //добавление случайного крестьянина к имеющимся (особое событие, возможное зимой)
	void Naim_krest(Village vil_naim, int numk); //найм крестьянина с выбранным номером из биржи труда (нумерация от единицы!)
